...
public:
	hand(int h, int m, int s, int l);
	void draw();
};
/********************************
函数功能：初始化结构体，角度和时间
********************************/
hand::hand(int h, int m, int s, int l)
{
	light = l;
	hour = h;
...
void hand::draw_hms(double len, double angle)
{
	...
	SDF_line(x1, y1, x2, y2, MY_PINK, 5);//指针的本体线条
	//SDF_circle(x2, y2, MY_PINK, NULL, 5, NULL);//短头的圆圈
	SDF_circle(CENTER_X, CENTER_Y, MY_RED, NULL, 10, 0, (20 - light / 2) + 5);//中心的圆圈 40
	SDF_circle(CENTER_X, CENTER_Y, MY_WHITE_2, NULL, 5, 0, light / 4 + 5);//20
	SDF_circle(x1, y1, MY_RED, NULL, 10, 0, light);//长头的圆圈 20
	SDF_circle(x1, y1, MY_WHITE_2, NULL, 4, 0, (10 - light / 4) + 5);//10
...
int main()
{
    ...
    while (!_kbhit()) {	
		...
		putimage(0, 0, background);
		struct hand h(t.tm_hour, t.tm_min, t.tm_sec, light);
		h.draw();
		//将图片particle渲染至屏幕
